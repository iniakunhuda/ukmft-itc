    ...
    <div class="box">   
        <div class="box-header with-border">
            ...
            <div class="box-tools pull-right">
                <a href="{{ route('kategori_karya.create') }}" class="btn btn-sm btn-primary">
        ...
        <div class="box-body">
            <table class="table table-bordered table-striped" data-table="datatable">
                ...
                <tbody>
                    @foreach($kategories as $key => $kat)
                    <tr>
                        <td>{{ $key+1 }}</td>
                        <td>{{ $kat->kategori }}</td>
                        <td>{{ $kat->tag }}</td>
                        <td>
                            <a href="{{ route('kategori_karya.edit', $kat->id) }}" class="btn btn-sm btn-primary">
                                <i class="fa fa-pencil" style="padding-right:10px"></i> Edit
                            </a>
                            <a href="javascript:;" 
                                class="btn btn-sm btn-danger" 
                                data-btn="delete" 
                                data-url="{{ route('kategori_karya.destroy', $kat->id) }}"
                                >
